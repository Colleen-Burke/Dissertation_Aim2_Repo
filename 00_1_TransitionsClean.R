...
Transitions1 <- read.csv(here("transitions_merged.csv"))
...
Transitions1 <- Transitions1 |>
  ...
  mutate(
    ...
    race_f = factor(
      race,
      levels = c(0, 1, 2, 3, 5),
      labels = c("White", "Black or African American", "Unknown", "Asian", "Other")
    ),
...
table(Transitions1$social_relationships_num, useNA = "ifany")
table(Transitions1$social_activities_num, useNA = "ifany")
table(Transitions1$social_function_num, useNA = "ifany")

summary(Transitions1$social_function_num)
